...
public sealed class AcademyOpsDbContextFactory
    : IDesignTimeDbContextFactory<AcademyOpsDbContext>
{
    private const string EnvironmentFileName = ".env";
    private const string UserSecretsId = "AcademyOps.Api";

    public AcademyOpsDbContext CreateDbContext(string[] args)
    {
        ...
        var optionsBuilder = new DbContextOptionsBuilder<AcademyOpsDbContext>();
        optionsBuilder.UseSqlServer(
            connectionString,
            options => options.MigrationsAssembly(migrationsAssemblyName));

        return new AcademyOpsDbContext(optionsBuilder.Options);
    }
    ...
    private static string ResolveApiProjectPath()
    {
        ...
        while (directory is not null)
        {
            var directCandidate = Path.Combine(directory.FullName, "AcademyOps.Api");
        ...
        throw new DirectoryNotFoundException(
            "Unable to locate AcademyOps.Api/appsettings.json.");
    }
}
