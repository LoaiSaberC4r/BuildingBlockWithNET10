...
namespace AcademyOps.Tests.Architecture;

public sealed class ArchitectureTests
{
    private static readonly Assembly Domain =
        AcademyOps.Domain.AssemblyReference.Assembly;
    private static readonly Assembly Application =
        AcademyOps.Application.AssemblyReference.Assembly;
    private static readonly Assembly Infrastructure =
        AcademyOps.Infrastructure.AssemblyReference.Assembly;
    private static readonly Assembly SqlServer =
        typeof(EmailOutboxMessage).Assembly;
    private static readonly Assembly Api =
        typeof(AcademyOps.Api.Configuration.AcademyOpsCorsOptions).Assembly;
    ...
    public void Domain_DoesNotReferenceOuterLayersOrFrameworks()
    {
        ...
        Assert.DoesNotContain("AcademyOps.Application", references);
        Assert.DoesNotContain("AcademyOps.Infrastructure", references);
        Assert.DoesNotContain(
            "AcademyOps.Infrastructure.EntityFrameworkCore.SqlServer",
            references);
        Assert.DoesNotContain("AcademyOps.Api", references);
        Assert.DoesNotContain(
            references,
    ...
    [Fact]
    public void BuildingBlockAssemblies_DoNotReferenceAcademyOps()
    {
        var assemblies = new[]
        {
    ...
    private static string FindRepositoryRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "AcademyOps.sln")))
            {
        ...
        throw new DirectoryNotFoundException(
            "Unable to locate the AcademyOps repository root.");
    }
